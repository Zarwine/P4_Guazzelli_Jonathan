...
if(!empty($_POST) && !empty($_POST['email']) ) {
    ...
    $req = $pdo->prepare('SELECT * FROM jf_users WHERE email = ? AND confirmed_at IS NOT NULL');
    ...
    if($user){
        ...
        $pdo->prepare('UPDATE jf_users SET reset_token = ?, reset_at = NOW() WHERE id = ?')->execute([$reset_token, $user->id]);
